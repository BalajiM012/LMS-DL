...
def test_home_ui_admin(client):
    ...
    login_response = client.post('/api/admin_login/login', data={'username': 'admin', 'password': 'password'}, follow_redirects=True)
    assert login_response.status_code == 200

    # Then access the admin page with follow_redirects=True
    response = client.get('/api/admin_login/admin', follow_redirects=True)
